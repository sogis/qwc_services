...
RUN set -x \
    ...
    && yum install -y --setopt=tsflags=nodocs python3-pip uwsgi uwsgi-plugin-python3 postgresql-dev python3-dev musl-dev \
    && yum remove -y $INSTALL_PKGS \
    && yum clean all \
    && rm -rf /var/cache/yum \
    && chown 1001:0 /home/jenkins /tmp \
    && groupadd -r www-data \
...
ENTRYPOINT uwsgi --http-socket :9090 --buffer-size $REQ_HEADER_BUFFER_SIZE --processes $UWSGI_PROCESSES \
    --threads $UWSGI_THREADS --plugins python36 --protocol uwsgi --wsgi-disable-file-wrapper --uid www-data \
